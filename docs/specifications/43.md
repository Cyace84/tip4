...
The index contract is deployed by the nft contract because the there is two index contracts per each nft contract.
Each index contract will have a specific code hash based on it's associated collection and owner.
In one of the index contracts address zero is passed as the collection address and in the other one the actual collection address is utilized.
