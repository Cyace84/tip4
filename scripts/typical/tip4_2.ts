...
import {
  factorySource,
  FactorySource,
} from "../artifacts/build_42/factorySource";
...
export async function deployTip4_2Collection(
  ...
  try {
    ...
    const collectionTvc: string = await (
      await fetch("/scripts/artifacts/build_42/Collection.base64")
    ).text();
    const nftTvc: string = await (
      await fetch("/scripts/artifacts/build_42/Nft.base64")
    ).text();
...
export async function deployTip4_2Nft(
